...
private:
	CPMDate* date;
	float hour, step_size, steps_per_day;
	int day_of_year;
...
public:
	...
	float get_hour() { return hour; }
	float get_step_size() { return step_size; }
	float get_steps_per_day() { return steps_per_day; }
	...
	Timer();
	Timer(float ss);
	Timer(int day, int month, int year, float ss);
